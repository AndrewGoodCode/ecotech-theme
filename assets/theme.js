  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _productThumbnailSlider: function() {
      ...
      if ($productThumbs.length > 1) {
        ...
        if (this.settings.sectionId === 'product-template-3' || this.settings.sectionId === 'product-template-4' || this.settings.sectionId === 'product-template-9'){
...
theme.Slideshow = (function() {
  ...
  function slideshow(el, sectionId) {
    ...
    var navslide = this.$slideshow.data('nav');
    var dotslide = this.$slideshow.data('dots');
    var countslide = this.$slideshow.data('counter');
    var pagi = this.$slideshow.data('pagi');
    ...
    this.settings = {
      ...
      autoplaySpeed: autoplaySpeed,
        customPaging: function (slider, i) {
          if(pagi){
            var targetImage = $('.slider-nav-thumbnails .thumbnails--item:nth-child(' + (i + 1) + ')').html();
            return '<div class="thumbnails--item">' + targetImage + '</div>';
          }else{
    ...
    if (countslide) {
      var $slider = this.$slideshow;
      $slider.on('init', function(event, slick){
        $(this).append('<div class="slider-count"><span id="current">01</span> / <span id="total">'+slick.slideCount+'</span></div>');
      });
      $slider.on('afterChange', function(event, slick, currentSlide, nextSlide){
            // finally let's do this after changing slides
            $('.slider-count #current').html(('0' + (currentSlide + 1)).slice(-2));
        });
    }
...
theme.Instagrams = (function() {
  function Instagrams(container) {
    ...
    var feed = new Instafeed({
      ...
      template		: '<div class="col mb-2 hv-image-brightness"><a class="instagram-item" href="{{link}}" target="_blank" id="{{id}} title="{{caption}}" ><img alt="{{caption}}" class="transition lazyload" data-src="{{image}}" /></a></div>',
      after			: afterInstagram,
      filter: function(image) {
          if (tag === "") {
            return true;
          } else {
            return image.caption.includes(tag);   
          }
      }
    });
...
theme.Producttabs = (function() {
  function Producttabs(container) {
    ...
    this.settings 	= {
      ...
      draggable			: this.$container.data('draggable') || false,
      infinite			: this.$container.data('infinite') || false,
      style			: this.$container.data('grid') || false,
    }
    this.settings.slidesToShow1200 = (this.settings.slidesToShow - 1) > 1? (this.settings.slidesToShow - 1) : 1;
    this.settings.slidesToShow992  = (this.settings.slidesToShow - 2) > 1? (this.settings.slidesToShow - 2) : 1;
    this.settings.slidesToShow768  = (this.settings.slidesToShow - 3) > 1? (this.settings.slidesToShow - 3) : 1;
    ...
    this.$container.find('a[data-bs-toggle="tab"]').on("shown.bs.tab", function(e) {
      _self._unSlick();
      _self._initSlick();
      theme.tooltip.load();
    })
...
theme.ParrallaxEffect = (function() {
  function ParrallaxEffect(container) {
    this.container = container;
      this.sectionId = $(container).attr('data-section-id');
      this.namespace = 'parallax-effect' + this.sectionId;
      var scene  =  document.getElementById('parallax-effect' + this.sectionId);
...
theme.init = function() {
  ...
  sections.register('parallax-effect',theme.ParrallaxEffect);
...
theme.miniCart = (function(){
  var miniCart = '.js-mini-cart',
      ...
      numberDisplayed = 5,
      svgcart = '<svg class="" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 86.8 77.5"><path d="M68.7 77.5c-5.6 0-10.1-4.5-10.1-10.1s4.5-10.1 10.1-10.1 10.2 4.5 10.2 10.1-4.6 10.1-10.2 10.1zm0-14.9c-2.6 0-4.7 2.2-4.7 4.8 0 2.6 2.2 4.7 4.8 4.7s4.7-2.2 4.7-4.8c0-2.5-2.2-4.7-4.8-4.7zM23.4 51.5l-11-46.1H0V0h16.7l11.1 46.1h45.6L80 19.9H48v-5.4h38.8l-9.1 37zM29.9 77.5c-5.5 0-10.1-4.7-10.1-10.2S24.5 57.2 30 57.2c5.6 0 10.1 4.6 10.1 10.2-.1 5.6-4.7 10.1-10.2 10.1zm0-14.9c-2.6 0-4.7 2.2-4.7 4.8 0 2.5 2.2 4.7 4.7 4.7 2.6 0 4.7-2.2 4.7-4.8 0-2.5-2.2-4.7-4.7-4.7z"/></svg>'
      emptyCartHTML = '<div class="alert mini-cart-empty"><div class="iconcart-empty">'+svgcart+'</div><div class="txtcart-empty">'+theme.strings.cartEmpty+'</div></div>';
  ...
  function updateElements(){
    Shopify.getCart(function(cart){
      if (cart.item_count === 0) {
        ...
        $cartBottom.addClass('hide');
      }else{
        $btnCheckout.removeClass('disabled');
        $cartBottom.removeClass('hide');
      }
    ...
    if (wishlistObject.length > 0){
      for (var i = 0; i < wishlistObject.length; i++) { 
        ...
        Shopify.getProduct(productHandle,function(product){
          ...
          htmlProduct += '	</a>';
          htmlProduct += '	<div><a href=""'+ product.url +'" class="title-prouct d-block h6" >'+ product.title +'</a>';
...
theme.crosssell = (function(){
  ...
  function showPopupCrosssell(lineItem){
    ...
    htmlLineItem += '<h6 class="cross-item-title">'+lineItem.title+'</h6>';
    ...
    if (arrayProduct.length > 0){
      ...
      for (var i = 0; i < arrayProduct.length; i++) {
        ...
        Shopify.getProduct(productHandle,function(product){
          ...
          htmlProduct += '		<h6 class="mb-2 mt-2">'+ product.title +'</h6>';
          htmlProduct += '	<span> '+ productPrice +'</span>';
...
theme.tooltip = (function(){
  var selector = '[data-bs-toggle="tooltip"],[data-tooltip="true"]';
